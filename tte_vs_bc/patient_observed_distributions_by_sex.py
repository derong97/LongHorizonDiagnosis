...
for index, label in enumerate(labels):
    ...
    for sex, sex_df in dfs.items():
        observed = get_observed(sex_df, label)
        try:
            print(f"sex {sex}: {np.median(observed)}")
            sns.kdeplot(observed, label=sex, ax=axes[index])
        except:
            print(f"sex {sex}: no data")
...
plt.tight_layout()
plt.savefig("data/results/patient_observed_distribution_by_sex.png")
