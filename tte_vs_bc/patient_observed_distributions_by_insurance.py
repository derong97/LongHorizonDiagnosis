...
import pandas as pd
import numpy as np
from utils import get_insurance_dfs
...
for index, label in enumerate(labels):
    ...
    dfs = get_insurance_dfs(df)

    for insurance, insurance_df in dfs.items():
        observed = get_observed(insurance_df, label)
        try:
            print(f"insurance {insurance}: {np.median(observed)}")
            sns.kdeplot(observed, label=insurance, ax=axes[index])
        except:
            print(f"insurance {insurance}: no data")
...
plt.tight_layout()
plt.savefig("data/results/patient_observed_distribution_by_insurance.png")
