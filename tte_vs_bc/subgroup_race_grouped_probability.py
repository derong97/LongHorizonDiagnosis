...
for i, label in enumerate(LABELS):
    ...
    df_boxplot = pd.DataFrame(
        index=range(df_size), columns=["Race", "Model Config", "Predicted Probability"]
    )
    ...
    for j, configuration in enumerate(CONFIGURATIONS):
        ...
        for race, race_probs in zip(races, probability_tmax):
            for indiv_prob in race_probs:
                df_boxplot.loc[idx] = [race, model_config, indiv_prob]
                idx += 1
...
plt.tight_layout()
plt.savefig(f"data/results/subgroup_race_grouped_probability.png")
