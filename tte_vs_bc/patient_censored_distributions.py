...
for index, label in enumerate(labels):
    ...
    censored = df[["mrn", f"censoring_age"]].groupby("mrn").first()

    # By year
    ax = axes[0, index]
    ax.hist(censored, bins=np.arange(11), color=myconfig_phenotype.color)
    ax.axvline(x=config_phenotype.age_cutoff, color="r")
    ax.set_xlabel("Censoring age (years)")
    ...
    ax.set_xlabel("Censoring age (months)")
    ax.set_ylabel("Frequency")
...
fig.suptitle(f"Distribution of Censoring Age", y=0.04, fontsize=20)
fig.tight_layout(rect=[0, 0.05, 1, 1])
plt.savefig("data/results/patient_censored_distribution.png")
