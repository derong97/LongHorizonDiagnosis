...
for index, label in enumerate(labels):
    ...
    for year, year_df in dfs.items():
        observed = get_observed(year_df, label)
        try:
            print(f"year {year}: {np.median(observed)}")
            sns.kdeplot(observed, label=year, ax=axes[index])
        except:
...
plt.tight_layout()
plt.savefig("data/results/patient_observed_distribution_by_yob.png")
