...
for i, label in enumerate(LABELS):
    ...
    df_boxplot = pd.DataFrame(
        index=range(df_size), columns=["Sex", "Model Config", "Predicted Probability"]
    )
    ...
    for j, configuration in enumerate(CONFIGURATIONS):
        ...
        for sex, sex_probs in zip(sexes, probability_tmax):
            for indiv_prob in sex_probs:
                df_boxplot.loc[idx] = [sex, model_config, indiv_prob]
                idx += 1
...
plt.tight_layout()
plt.savefig(f"data/results/subgroup_sex_grouped_probability.png")
